...
type
  ...
  TFbCharsetHelper = record helper for TFBCharSet
    ...
    function GetString(const Bytes: TBytes; ByteIndex, ByteCount: Integer): String;
    function GetBytes(const aInput: String): TBytes;
  end;
...
function TFbCharsetHelper.GetBytes(const aInput: String): TBytes;
 var
  LEncoding: TEncoding;
begin
  LEncoding := nil;
  try
    LEncoding := GetEncoding();
    Result := LEncoding.GetBytes(aInput);
  finally
    LEncoding.Free;
  end;
end;
...
begin
  Result := String(CharSetMap[Integer(Self)].CharSetName);
end;
...
function TFbCharsetHelper.GetString(const Bytes: TBytes; ByteIndex, ByteCount: Integer): string;
 var
  LEncoding: TEncoding;
begin
  LEncoding := nil;
  try
    LEncoding := GetEncoding();
    Result := LEncoding.GetString(Bytes, ByteIndex, ByteCount);
  finally
    LEncoding.Free;
  end;
end;
