    ...
    case C of
      ...
      begin
        ...
        begin
          ...
          Result := Result + RawByteString(LowerCase(IntToHex(Codepoint, 4)));
        end
        ...
        begin
          ...
          Assert(Codepoint > $FFFF);
          Result := Result + RawByteString(LowerCase(IntToHex($D7C0 + (Codepoint shr 10), 4)));
          Result := Result + RawByteString(LowerCase(IntToHex($DC00 + (Codepoint and $3FF), 4)));
        end;
      end
      {$ELSE}
      if (C < ' ') or (C >= #$0080) then
